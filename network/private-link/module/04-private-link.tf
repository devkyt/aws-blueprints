resource "aws_vpc_endpoint_service" "private_link_service" {
  ...
  tags = merge(local.tags, { Name = "${local.private_link_name}-service" })
}
...
resource "aws_vpc_endpoint" "private_link_endpoint" {
  ...
  security_group_ids = [aws_security_group.private_link_endpoint_sg.id]

  tags = merge(local.tags, { Name = "${local.private_link_name}-endpoint" })
}
